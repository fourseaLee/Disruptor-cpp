...
namespace Engine
{
    ...
    OneToOneRawThroughput::OneToOneRawThroughput()
    {
    ...
    std::int64_t OneToOneRawThroughput::run(Stopwatch& stopwatch)
    {
        m_taskScheduler->start(requiredProcessorCount());
        UtilTools::ScopeExitFunctor atScopeExit([this] { m_taskScheduler->stop(); });

        auto latch = std::make_shared< Util::ManualResetEvent >(false);
    ...
    std::int32_t OneToOneRawThroughput::requiredProcessorCount() const
    {
        return 2;
    }

    void OneToOneRawThroughput::waitForEventProcessorSequence(std::int64_t expectedCount) const
    {
    ...
    OneToOneRawThroughput::MyRunnable::MyRunnable(const std::shared_ptr< ISequencer< boost::any > >& sequencer)
    {
        m_barrier = sequencer->newBarrier({});
    }

    void OneToOneRawThroughput::MyRunnable::reset(const std::shared_ptr< Util::ManualResetEvent >& latch, std::int64_t expectedCount)
    {
        m_latch = latch;
        m_expectedCount = expectedCount;
    }

    void OneToOneRawThroughput::MyRunnable::run() const
    {
