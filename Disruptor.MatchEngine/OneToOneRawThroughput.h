...
namespace Engine
{
     ...
     * 
     */
    class OneToOneRawThroughput : public IThroughput
    {
        //C# Run: Ops:  140 078 898 - Duration: 1 428 (ms)
        class MyRunnable;

    public:
        OneToOneRawThroughput();
...
//#endif
    ...
    class OneToOneRawThroughput::MyRunnable
    {
    public:
        explicit MyRunnable(const std::shared_ptr< ISequencer< boost::any > >& sequencer);

        void reset(const std::shared_ptr< Util::ManualResetEvent >& latch, std::int64_t expectedCount);
    ...
    private:
        std::shared_ptr< Util::ManualResetEvent > m_latch;
